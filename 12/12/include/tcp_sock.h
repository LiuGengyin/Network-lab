...
#define CWND_INFINITY 1000000000

extern int who;

extern int total;
extern int packs_sent, packs_rcvd;
...
#define sk_dport peer.port
  ...
  //gy: check the notebook to get clear about the hierarchy.

	// represents the number that the tcp sock is referred, if this number 
	// decreased to zero, the tcp sock should be released
	int ref_cnt;
  //gy: the "reference" means a reference from the protocol stack. increased when sock inserted into hash, and decrease 
  ...
  //gy: see tcp_hash.h. each element in those arrays is a listhead. link these to the correspond listhead calculated by hash.
...
#define TCP_MAX_BACKLOG 128
	...
	int backlog;
  //gy: should this be initialized as TCP_MAX_BACKLOG?
