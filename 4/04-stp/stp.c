		...
		struct stp_config *config)
{
    ...
    if(p->designated_root != ntohll(config->root_id)){
        if(p->designated_root < ntohll(config->root_id)) i_am_prior = 1;
    }
    else if(p->designated_cost != ntohl(config->root_path_cost)){
        if(p->designated_cost < ntohl(config->root_path_cost)) i_am_prior = 1;
    }
    else if(p->designated_switch != ntohll(config->switch_id)){
        if(p->designated_switch < ntohll(config->switch_id)) i_am_prior = 1;
    }
    else if(p->designated_port != ntohs(config->port_id)){
        if(p->designated_port < ntohs(config->port_id)) i_am_prior = 1;
    }
    else i_am_prior = 1;
    ...
    if(!i_am_prior){
        ...
        for(int i = 0; i < stp->nports; i++){
            if(!stp_port_is_designated(&stp->ports[i])){
                if(root){
                    int this_is_prior = 0;
                    if(stp->ports[i].designated_root != root->designated_root){
                        if(stp->ports[i].designated_root < root->designated_root) this_is_prior = 1;
                    }
                    else if(stp->ports[i].designated_cost != root->designated_cost){
                        if(stp->ports[i].designated_cost < root->designated_cost) this_is_prior = 1;
                    }
                    else if(stp->ports[i].designated_switch != root->designated_switch){
                        if(stp->ports[i].designated_switch <root->designated_switch) this_is_prior = 1;
                    }
                    else if(stp->ports[i].designated_port != root->designated_port){
                        if(stp->ports[i].designated_port < root->designated_port) this_is_prior = 1;
                    }
                    else this_is_prior = 1;
        ...
        for(int i = 0; i < stp->nports; i++){
            if(!stp_port_is_designated(&stp->ports[i])){
                int this_is_prior = 0;
                if(stp->designated_root != stp->ports[i].designated_root){
                    if(stp->designated_root < stp->ports[i].designated_root) this_is_prior = 1;
                }
                else if(stp->root_path_cost != stp->ports[i].designated_cost){
                    if(stp->root_path_cost < stp->ports[i].designated_cost) this_is_prior = 1;
                }
                else if(stp->switch_id != stp->ports[i].designated_switch){
                    if(stp->switch_id < stp->ports[i].designated_switch) this_is_prior = 1;
                }
                else if(stp->ports[i].port_id != stp->ports[i].designated_port){
                    if(stp->ports[i].port_id < stp->ports[i].designated_port) this_is_prior = 1;
                }
                else this_is_prior = 1;
        ...
        //4
        if(!stp_is_root_switch(stp)) stp_stop_timer(&stp->hello_timer);
