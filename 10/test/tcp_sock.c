...
// decreased to zero.
void free_tcp_sock(struct tcp_sock *tsk)               //contemplate: when should free_sock be called? at least it should only be called on parent socks.
{
	  //fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    tsk->ref_cnt -= 1;
    if(tsk->ref_cnt < 0) return;
    if(tsk->ref_cnt == 0){
        //if(!list_empty(&tsk->hash_list)) list_delete_entry(&tsk->hash_list); //see tcp_unhash
        //if(!list_empty(&tsk->bind_hash_list)) list_delete_entry(&tsk->bind_hash_list); //see tcp_bind_unhash
        assert(list_empty(&tsk->listen_queue));
        assert(list_empty(&tsk->accept_queue));
        ...
        free(tsk->rcv_buf); //go back here in part 2
        pthread_mutex_destroy(&tsk->rcv_buf_lock);
        /*deal with tsk->send_buf
        deal with tsk->rcv_ofo_buf*/ //go back here in subsequence exps
...
{
	  //fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    struct tcp_sock *res = NULL;
    int index = tcp_hash_function(saddr, daddr, sport, dport);
    struct list_head *listhdr = &tcp_established_sock_table[index];
    struct tcp_sock *entry_tsock;
    list_for_each_entry(entry_tsock, listhdr, hash_list){
        if(entry_tsock->sk_sip == saddr &&
           entry_tsock->sk_dip == daddr &&
           entry_tsock->sk_sport == sport &&
           entry_tsock->sk_dport == dport){
               res = entry_tsock;
               break;
        }
    }
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
    ...
    struct list_head *listhdr = &tcp_listen_sock_table[index];
    struct tcp_sock *entry_tsock;
    list_for_each_entry(entry_tsock, listhdr, hash_list){
        if(entry_tsock->sk_sport == sport){
            res = entry_tsock;
            break;
        }
    }
...
struct tcp_sock *tcp_sock_lookup(struct tcp_cb *cb)
{
  ...
  //if(tsk) log(DEBUG, "found child in est table");
	if (!tsk)
...
static int tcp_sock_set_sport(struct tcp_sock *tsk, u16 port)
{
	if ((port && tcp_port_in_use(port)) ||
			(!port && !(port = tcp_get_port())))           //that means if you pass 0 for the arg port, this func will alloc a free port for tsk.
		return -1;
...
// 3. send SYN packet, switch to TCP_SYN_SENT state, wait for the incoming        //use: tcp_send_control_packet in tcp_out.c
...
{
	  //fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    rt_entry_t *rentry = longest_prefix_match(ntohl(skaddr->ip));
    if(!rentry) return -1;
    
    tsk->sk_sip = rentry->iface->ip;
    tsk->sk_sport = tcp_get_port();
    if(!tsk->sk_sport) return -2;
    tsk->sk_dip = ntohl(skaddr->ip);
    tsk->sk_dport = ntohs(skaddr->port);
    tsk->mybuf = (struct buffer *)malloc(sizeof(struct buffer));
    init_buff(tsk->mybuf, 5000000);
...
{
	  //fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    if(tsk->state != TCP_CLOSED) return -2;
    tsk->backlog = backlog;
    tsk->accept_backlog = 0;
    tcp_set_state(tsk, TCP_LISTEN);
    if(tcp_hash(tsk) < 0) return -1;
...
{
	  //fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    struct tcp_sock *rdy_sock = NULL;
    
    while(list_empty(&tsk->accept_queue)){
        sleep_on(tsk->wait_accept);
    }
    
    rdy_sock = tcp_sock_accept_dequeue(tsk);
    assert(rdy_sock);
    //rdy_sock->state = TCP_ESTABLISHED;
    //tcp_hash(rdy_sock);                //do these when the sock is inserted into accept_queue.
...
{
    //fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    if(tsk->state == TCP_LISTEN){ //server: tsk is parent. it's not a established link, so no need to notify peer.
        tcp_unhash(tsk);
        tcp_bind_unhash(tsk);
        //danger: tsk may has childs in its listen or accept queue!
    }
    else if(/*(tsk->state == TCP_SYN_RECV) || */(tsk->state == TCP_ESTABLISHED)/* || (tsk->state == TCP_SYN_SEND)*/){ //server/client: (tsk is child.) notify peer.
        tcp_send_control_packet(tsk, TCP_FIN);
        if(tsk->state != TCP_SYN_SENT) tcp_set_state(tsk, TCP_FIN_WAIT_1);
        else tcp_set_state(tsk, TCP_CLOSED);
    }
...
//assume that len is the length the caller wants to read(in a batch).
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len){
    if(tsk->state == TCP_CLOSED) return 0; //RST
    
    pthread_mutex_lock(&tsk->rcv_buf_lock);
    while(buff_empty(tsk->mybuf)){ //what if received out-of-order packs before?
    ...
    int rlen = min(len, buff_used(tsk->mybuf)); //similarly, what if out-of-order?
    read_buff(tsk->mybuf, buf, rlen);
...
//assume that len is the total length the caller wants to transmit.
int tcp_sock_write(struct tcp_sock *tsk, char *buf, int len){
    if(tsk->state == TCP_CLOSED) return 0;
    
    int chars_rmn = len;
    while(chars_rmn > 0){
        int this_len = min(chars_rmn, 1514 - sizeof(struct ether_header) - IP_BASE_HDR_SIZE - TCP_BASE_HDR_SIZE);
        
        //log(DEBUG, "eff_wnd is %d, this_len is %d", tsk->snd_wnd - (tsk->snd_nxt - tsk->snd_una), this_len);
        while(less_than_32b(tsk->snd_wnd - (tsk->snd_nxt - tsk->snd_una), this_len)){
            tsk->snd_wnd = 0;
            sleep_on(tsk->wait_send);
        }
        //log(DEBUG, "send a packet");
        
        char *packet = (char *)malloc(sizeof(struct ether_header) + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + this_len);
        memcpy(packet + sizeof(struct ether_header) + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE, buf, this_len);
        tcp_send_packet(tsk, packet, sizeof(struct ether_header) + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + this_len);
