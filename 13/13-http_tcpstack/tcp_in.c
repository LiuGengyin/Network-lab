...
void tcp_process(struct tcp_sock *tsk, struct tcp_cb *cb, char *packet)
{
    ...
    if(tsk->state == TCP_LISTEN){ //server: tsk is parent
        ...
        if(is_tcp_seq_valid(tsk, cb)){
            ...
            if(less_than_32b(ring_buffer_free(tsk->rcv_buf), cb->pl_len)){
                log(DEBUG, "drop pack for overflow");
                pthread_mutex_unlock(&tsk->rcv_buf_lock);
                return;
            }
            if(cb->pl_len == 0){ //third handshake
                //if(who == 0) log(DEBUG, "rcv len 0");
                wake_up(tsk->wait_recv);
                pthread_mutex_unlock(&tsk->rcv_buf_lock);
                return;
            }
            if(cb->seq != tsk->rcv_nxt){ //packs not sequent
                //log(DEBUG, "why?");
            ...
            tsk->rcv_wnd = ring_buffer_free(tsk->rcv_buf);
            //if(cb->seq_end != cb->pl_len + cb->seq) log(DEBUG, "!=");
            tsk->rcv_nxt = cb->seq_end;
            wake_up(tsk->wait_recv);
            tcp_send_control_packet(tsk, TCP_ACK);
            pthread_mutex_unlock(&tsk->rcv_buf_lock);
